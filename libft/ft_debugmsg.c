...
/*   Updated: 2024/01/15 14:50:14 by ivalimak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_printarg(char *f, va_list args);
...
void	ft_debugmsg(char *pfx, char *f, ...)
{
	...
	{
		if (*f != '%')
			f = f + ft_dprintf(DEBUGFD, "%.*s", ft_strclen(f, '%'), f);
		else
			f = ft_printarg(f + 1, args);
	}
	ft_putstr_fd(DBGRESET, DEBUGFD);
	ft_putchar_fd('\n', DEBUGFD);
}
